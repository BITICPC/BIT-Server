        ...
        <b-row>
          <b-col>
            <b-form @submit="saveChanges">
              <b-form-group
                label="Input file:"
                description='Input file name or "stdin" for standard input'>
                <b-form-input placeholder="stdin" disabled></b-form-input>
              </b-form-group>
              <b-form-group
                label="Output file:"
                description='Output file name or "stdout" for standard output'>
                <b-form-input placeholder="stdout" disabled></b-form-input>
              </b-form-group>
              <b-form-group
                label="Time Limit:"
                description="Time limit per test (between 1000 ms and 10000 ms)">
              ...
              <b-form-group
                label="Memory Limit:"
                description="Memory limit (between 32 MB and 1024 MB)">
              ...
              <b-form-group
                label="Judge Mode:"
                description='The mode to judge the problem (from "standard I/O" or "interactive" or "special judge")'>
          ...
          <b-col class="bd-toc col-xl-3 d-none d-xl-block">
                ...
                <b-card-text>
                  <b-row>
                    ...
                    <b-col style="padding-left: 3.5px; padding-right: 10px;">
                      <template v-if="problem.tags.length == 0">
                      ...
                      >
                        {{ tag }}
                        <button type="button" class="close" v-on:click="eraseTag(index)">×</button>
                      </b-badge>
                    </b-col>
                  </b-row>
                  <div style="margin-top: 10px;">
                    <b-form-select v-model="tagSelected" :options="tagOptions" size="sm">
                      <template v-slot:first>
                  ...
                  <b-row style="margin-top: 10px;">
                    <b-col cols="5" style="padding-right: 0px;">
                      <b>Difficulty:</b>
                    </b-col>
                    <b-col>
                      <b-form-input
            ...
            <template v-if="!!problem.archiveId">
              <b-button
                ...
                block
              >Change into private</b-button>
            </template>
            <template v-else>
              <b-button
                variant="info"
                style="margin-top: 10px;"
                v-b-modal.make-public
                block
...
export default {
  ...
  mounted() {
    api
      .getProblemDetail(this.$route.params.id, this.$store.state.user.jwt)
      .then(res => {
  ...
  methods: {
    ...
    statementValidation() {
      return (
        ...
        this.problem.legend.length > 0 &&
        this.problem.input.length > 0 &&
        this.problem.output.length > 0
      );
    },
    validation() {
      this.formErrors.errMessage = [];
      if (!this.statementValidation()) {
        this.formErrors.errMessage.push(
          "problem statement should not be empty."
        );
      }
      if (!this.problem.testReady) {
        this.formErrors.errMessage.push(
          "There are no test points available for this problem."
        );
      }
      if (!this.problem.archiveId) {
        this.formErrors.errMessage.push(
          "The problem has not been made public, it will not appear in the public archive list."
        );
      }
    ...
    handleSubmit() {
      api
        ...
        .then(res => {
          this.$bvToast.toast(
            "You have successfully added this problem into the public problem set.",
            {
              title: "Polygon message",
              toaster: "b-toaster-bottom-right",
              variant: "success",
    ...
    showConfirmation() {
      this.$bvModal
        ...
        .then(value => {
          if (value) {
            api
              .deleteFromPublicProblemList(
                [this.problem.archiveId],
                this.$store.state.user.jwt
              )
              .then(res => {

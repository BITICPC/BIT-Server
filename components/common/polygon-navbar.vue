  ...
  <div>
        ...
        <template v-else>
          <v-menu open-on-hover offset-y>
            <template v-slot:activator="{ on }">
              <v-btn class="nav" text :to="{ path: `/user/${profile.username}`, query: { polygon: true } }" v-on="on">
    ...
    <v-navigation-drawer id="drawer" v-model="drawer" width="200" temporary app>
      <template v-if="isLogin" v-slot:prepend>
        <v-list-item :to="{ path: `/user/${profile.username}/setting`, query: { polygon: true } }" two-line>
          <v-list-item-avatar>
            <v-img :src="avatar" />
          </v-list-item-avatar>
          <v-list-item-content>
            ...
            <v-list-item-subtitle>
              {{ profile.role ? 'Admin' : 'Regular User' }}
            </v-list-item-subtitle>
          </v-list-item-content>
        </v-list-item>
        <v-divider />
